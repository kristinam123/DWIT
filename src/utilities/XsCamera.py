...
#   Version 2.16.08
#   Copyright (C) Integrated Design Tools, Inc.
#   ALL RIGHTS RESERVED
###############################################################################
...
class XS_CAM_MODEL(XSULONG32):
    ...
    XS_CM_CC_M5K05  = 39           # CrashCam Mini 5K - OSII 5K
    XS_CM_MP_O3     = 40           # MotionXtra Os3
    XS_CM_CC_M1520  = 41           # CrashCam Mini 1520
    XS_CM_CC_M3510  = 42           # CrashCam Mini 3510
    XS_CM_MINI_POV  = 43           # Mini POV
    XS_CM_CC_M1540  = 44           # CrashCam Mini 1540/1550 (see clock) - OSII 1550 (see XS_MISC_CAPS)
    XS_CM_CC_M3525  = 45           # CrashCam Mini 3525/3530 (see clock) - OSII 3530 (see XS_MISC_CAPS)
    XS_CM_PCIE_X7   = 46           # PCIe X-Stream 720p
    XS_CM_PCIE_X14  = 47           # PCIe X-Stream 1440p
    XS_CM_XSM_1540  = 48           # XSM 1540 - XSII 1550 (see XS_MISC_CAPS)
    XS_CM_XSM_3520  = 49           # XSM 3520 - XSII 3530 (see XS_MISC_CAPS)
    XS_CM_XSM_STICK = 50           # XS Stick
    XS_CM_XSM_5K    = 51           # XSM 5K - XSII 5K (see XS_MISC_CAPS)
    XS_CM_XSM_4KV   = 52           # XSM 4K-Veloce - XSII 4K-Veloce (see XS_MISC_CAPS)
    XS_CM_XSM_HD    = 53           # XSM HD - XSII HD (see XS_MISC_CAPS)
    XS_CM_XSM_PHD   = 54           # Phoenix HD
    XS_CM_R_2K      = 55           # R-series 2K
    XS_CM_CC_STICK  = 56           # CrashCam Stick
    XS_CM_XSM_P4K   = 57           # Phoenix 4K
    XS_CM_XSM_8KP   = 58           # XSII 8K Plus
    XS_CM_CC_MHD    = 59           # Crash Cam Mini HD - OSII HD
    XS_CM_CCM_4KV   = 60           # Crash Cam Mini 4K-Veloce
    XS_CM_CCM_19NN  = 61           # Crash Cam Mini 1970/1925
    XS_CM_XSM_P6KP  = 62           # Phoenix Gold 6K Plus - Galileo 6K (see XS_MISC_CAPS)
    XS_CM_XSM_P5KP  = 63           # Phoenix Gold 5K Plus - Galileo 5K (see XS_MISC_CAPS)
    XS_CM_XSM_4KP   = 64           # XSII 4K Plus
    XS_CM_CCM_35XX  = 65           # OSII 35XX (GE link)
    XS_CM_XSM_35XX  = 66           # OSII 35XX (TB Link)
    XS_CM_XSM_G3K   = 67           # Galileo 3K

# camera name/model max lenght
L_CAMNAME = 128
L_CAMMODEL = 64
...
class XS_ENUM_FLT(XSULONG32):
    ...
    XS_EF_REM   = 0x00000200        # Remote XSM/PCIe (via ethernet)
    XS_EF_VCAM  = 0x00010000        # Virtual camera (RAW sequence)
    XS_EF_VSSD  = 0x00020000        # Virtual camera (removable SSD, SD card)
    XS_EF_VRCAM = 0x00040000        # Virtual remote camera (remote RAW)
    #
    XS_EF_ALL   = 0x00000FFF        # mask
...
# Frame Grabber Type (Camera Link) or XS-RT Board Type (TB/PCI)
class XS_FG_TYPE(XSULONG32):
    ...
    XS_RT_TB_8GB      = 5           # XS-RT TB3 external box (8 GB DDR)
    XS_RT_TB_16GB     = 6           # XS-RT TB3 external box (16 GB DDR)
    XS_RT_PCI_16GB    = 7           # XS-RT PCI internal board (16 GB DDR)
    XS_RT_PCI_H2GB    = 8           # XS-RT PCI high-speed internal board (2 GB DDR)
    XS_RT_PCI_H2GBD   = 9           # XS-RT PCI high-speed dual internal board (2 GB DDR each)
    XS_RT_PCI_U8GB    = 10          # XS-RT PCI ultra-speed internal board (8 GB DDR - 100 Gbps)
    XS_RT_PCI_UD8GB   = 11          # XS-RT PCI ultra-speed internal board (8 GB DDR - 100 Gbps - dual)
    XS_RT_PCI_UD16GB  = 12          # XS-RT PCI ultra-speed internal board (16 GB DDR - 200 Gbps - dual)
    XS_RT_PCI_UREC    = 13          # XS-RT PCI ultra-speed internal recorder (100 Gbps)
...
class XS_SNS_MODEL(XSULONG32):
    ...
    XS_SM_4KVEL         = 17        # 4K veloce sensor
    XS_SM_PHNX_HD       = 18        # Phoenix HD sensor
    XS_SM_PHNX_4K       = 19        # Phoenix 4K sensor
    XS_SM_EM67M         = 20        # Emerald 67M
    XS_SM_NORTH_STAR_III= 21        # North Star II (CCM-1960)
    XS_SM_PHNX_6KP      = 22        # Phoenix Gold 6K Plus / Galileo 6K sensor
    XS_SM_PHNX_5KP      = 23        # Phoenix Gold 5K Plus / Galileo 5K sensor
    XS_SM_35XX          = 24        # 35XX sensor
    XS_SM_G3K           = 25        # Galileo 3K sensor
...
class XS_MISC_CAPS(XSULONG32):
    ...
    XS_CAP_OSII   = 0x00400000      # the camera is OS-II
    XS_CAP_REMOTE = 0x00800000      # the camera is Remote XSM/PCIe
    XS_CAP_GEV    = 0x01000000      # the camera supports GigE Vision protocol
    XS_CAP_CLIP   = 0x02000000      # the camera is a clip (raw genrated by other software)
    XS_CAP_XSMII  = 0x04000000      # the camera is XSM-II (dual)
    XS_CAP_GALILEO= 0x08000000,     # the camera is Galileo (rugged)
...
def CAP_IS_OSII(c):
    return ((c)&XS_CAP_OSII).value
def CAP_IS_XSMII(c):
    return ((c)&XS_CAP_XSMII).value
def CAP_IS_GALILEO(c):
    return ((c)&XS_CAP_GALILEO).value
...
class XS_PRE_PARAM(XSULONG32):
    ...
    XSPP_ENABLE_8BIT_ADD = 11       # enable 8 bit addressing on CCM cameras
...
class XS_SYNCIN_CFG(XSULONG32):
    ...
    XS_SIC_EPLL_PULSE_HI = 24       # external pulse high - phase lock loop
    XS_SIC_EPLL_PULSE_LO = 25       # external pulse low - phase lock loop
...
class XS_CALIB_OPCODE(XSULONG32):
    ...
    XS_C_MEMORY        = 6          # Memory calibration (CC-Mini)
...
# Lens Commands
class XS_LENS_CMD(XSULONG32):
    XS_LCMD_POWEROFF = 0            # lens power off
    XS_LCMD_RESET = 1               # lens reset

# Write to disk (streaming) options
class XS_W2D_OPT(XSULONG32):
    W2DO_START_FREE = 0,            # start immediately and stop on trigger, abort or when a defined number of frames has been saved
    W2DO_START_TRIG = 1,            # start with first trigger and stop on trigger, abort or when a defined number of frames has been saved

# Supported PTP modes
class XS_PTP_SUPPORT(XSULONG32):
    PTP_S_ETH_SLV = 0,              # Ethernet Slave mode is supported
    PTP_S_ETHUDP_SLV = 1,           # Ethernet Slave and UDP Slave modes are supported
    PTP_S_UDP_SLV = 2,              # UDP Slave mode is supported

# Operations during camera open
class XS_OPEN_CODE(XSULONG32):
    XS_OP_ATTACH = 0,               # attach, start communication
    XS_OP_ALLOC_MEM = 1,            # allocate memory
    XS_OP_INIT_MEM = 2,             # init memory
    XS_OP_INIT_FLASH = 3,           # init internal flash meomry
    XS_OP_INIT_REGS = 4,            # init internal registers
    XS_OP_INIT_CFG = 5,             # init camera configuration
    XS_OP_INIT_DRV = 6,             # init driver
    XS_OP_ALLOC_DRVM = 7,           # allocate driver memory
    XS_OP_DONE = 8,                 # open process is done
...
class XS_ERROR(XSULONG32):
    ...
    XS_E_FILE_OPEN         = 22     # Unable to open file for streaming to local disk
    XS_E_DISK_FULL         = 23     # Local disk is full (unable to keep streaming)
    XS_E_STREAM_STOP       = 24     # Streaming has been stopped (not an error, a notification)
...
class XS_PARAM(XSULONG32):
    ...
    XSP_DYNAMIC_NR   = 54           # TNK: temporal noise killer [0 to 40] dfl = camera dependent
    XSP_SHARPEN      = 55           # sharpening [0 to 7] dfl = camera dependent
    XSP_BRIGHTNESS   = 56           # brightness [0 to 50] dfl = 25
    XSP_CONTRAST     = 57           # contrast [0 to 20] dfl = 10
    XSP_HUE          = 58           # hue [0 to 360] dfl = 180
    XSP_SATURATION   = 59           # saturation [0 to 20] dfl = camera dependent
    ...
    XSP_SHARPEN_THR    = 104        # sharpening threshold [0 to 128]
    XSP_SYNCIN_INV     = 105        # Sync in polarity inversion
    XSP_DYNAMIC_NR2    = 106        # space dynamic noise reduction [0 to 30] dfl = camera dependent
    ...
    XSP_SYNCIN_F_MUL   = 134        # sync in frequency multiplier in external PLL mode
    ...
    XSP_LENS_FOCUS_REL = 153        # Lens focus delta
    XSP_LENS_CMD       = 154        # Lens command (see XS_LENS_CMD list)
    XSP_STRM_MAX_PTRG  = 155        # XSM streaming max pre-trigger frames (read-only)
    XSP_MAX_FPS        = 156        # Max Fps for current settings
    XSP_AE_MIN_IRIS    = 157        # set the value of min aperture in Auto-Iris mode
    XSP_AE_MAX_IRIS    = 158        # set the value of max aperture in Auto-Iris mode
    XSP_PLL_IS_LOCKED  = 159        # return 1 if PLL sync is locked, otherwise 0
    XSP_SYNCIN_F_DIV   = 160        # sync in frequency divider in external PLL mode
...
class XS_INFO(XSULONG32):
    ...
    XSI_VIDEO_OUT           = 33    # the camera supports video output (SDI, HDMI)
    ...
    XSI_PIPELINE            = 60    # the camera has internal pipeline for image rendering.
    ...
    XSI_PTP_MODE_SUPPORT    = 87    # supported PTP modes (see XS_PTP_SUPPORT)
    XSI_LENS_INFO           = 88    # lens information
    XSI_REMOTE_CAM          = 89    # camera is remote (XSM connected via Eth)
    XSI_CAMERA_MODEL_STR    = 90    # camera model string (zero terminated)
    XSI_AUTO_IRIS           = 91    # the camera supports auto-iris
    XSI_LENS_NAME           = 92    # returns the lens name
...
class XS_ENUMITEM(ctypes.Structure):
    _fields_ = [
        ...
        ("szCameraMod",    ctypes.c_char * L_CAMMODEL),   # camera model string
        ("anFill",    XSULONG32 * 16)        # fill parameters, for future use
        ]
    def __init__(self):
...
XS_StreamingCallback = CALLBACK_TYPE(
    ...
    XSULONG32,       # Amount of available computer DDR buffer (0 to 1000)
    XSULONG32,       # Amount of available xs-rt DDR (0 to 1000)
    XSULONG32        # Error code
)

# camera open callback
XS_OpenCallback = CALLBACK_TYPE(
    None,            # No return is expected from callback
    ctypes.c_void_p, # User defined variable
    XSULONG32,       # ID of camera that is opening
    XSULONG32,       # code of operation currently executing (see XS_OPEN_CODE values)
    XSULONG32,       # Current operation parameter (depends on op value)
    XSULONG32        # current progress [0 to 1000]
)
...
class CameraError(Exception):
    ...
    def __init__(self, code):
        self.code = code
        if code in self.error_code_details:
    ...
    @classmethod
    def Create(cls, code):
        if code in cls.error_code_details:
...
#*  XsSetOpenCallback
#*
#* Descr: Sets the callback that will be called in camera open process.
#*
#* Param: pfnCallback - callback routine pointer, it may be NULL.
#*        pUserData - parameter returned in callback, pointer to user data.
#*
#* Ret:   None
#*
#* Exceptions:
...
#*****************************************************************************
#
def XsSetOpenCallback(pfnCallback,pUserData):
    error = XStreamDrv.XsSetOpenCallback(pfnCallback,pUserData)
    _xs_error_check(error)
...
def XsPreConfigCamera(nCameraId, nParamKey, *value):
    ...
    if nParamKey == XS_PRE_PARAM.XSPP_IP_ADDRESS:
        ...
        nValueLo.value = _ip_param_to_num(value[0])
                       ...
                       XS_PRE_PARAM.XSPP_PCIX_DMASIZE,
                       XS_PRE_PARAM.XSPP_ENABLE_8BIT_ADD]:
...
#*
#* Param: hCamera - handle to an open camera
#*        pfnCallback - callback routine pointer, it may be NULL.
#*        pUserData - parameter returned in callback, pointer to user data.
#*
#* Ret:   None
#*
...
#*  XsImageConversion
#*
#* Descr: converts a Bayer image in RGB or BGR
#*
#* Param: pOutBuf  - output RGB data buffer
#*        pInBuf - input bayer data buffer
#*        nInPixDepth -  input pixel depth (0, 10, 12).
#*        nWidth - image width in pixels
#*        nHeight - image height in pixels
#*        nBayPattern - input image CFA Bayer pattern (see XS_CFA_PATTERN)
#*        nOutPixDepth - output pixel depth (24, 30, 36).
#*        nPlusMode - input is plus mode image (half height)
#*        nRGB - 0:output image is BGR,  1:output image is RGB
#*        pnScaleMin - pointer to min value
#*        pnScaleMax - pointer to max value
#*        nAutoScale - 1: compute min and max, scale image and put them into pnScaleMin and pnScaleMax
#*                     0: use values in pnScaleMin and pnScaleMax to scale image
#*
#* Ret:   XS_SUCCESS - resizer successfully activated
#*        XS_E_INVALID_ARGUMENTS - invalid arguments
#*        XS_E_NOT_SUPPORTED - the routine is not supported
...
#*****************************************************************************
#
def XsImageConversion(pOutBuf, pInBuf, nInPixDepth, nWidth, nHeight, nBayPattern, nOutPixDepth, nPlusMode, nRGB, nScaleMin, nScaleMax, nAutoScale):
    error = XStreamDrv.XsImageConversion(pOutBuf, pInBuf, nPixDepth, nWidth, nHeight, nBayPattern, nOutPixDepth, nPlusMode, nRGB, nScaleMin, nScaleMax, nAutoScale)
    _xs_error_check(error)
